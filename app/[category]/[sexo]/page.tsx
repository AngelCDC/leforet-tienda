// app/category/[category]/[sexo]/page.tsx
...
export async function generateMetadata({ params }: { params: { category: string; sexo: string } }): Promise<Metadata> {
  // Validar antes de generar metadata
  const isValid = await validateCategoryAndSexo(params.category, params.sexo);
  if (!isValid) {
    return {
...
export default async function CategoryPage({
  ...
  // ✅ VALIDACIÓN CLAVE: Verificar que ambos parámetros existan
  const isValid = await validateCategoryAndSexo(params.category, params.sexo);
  if (!isValid) {
    notFound(); // Esto redirige a not-found.tsx
  }
